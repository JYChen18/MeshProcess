...
def task_wrapper(func):
    def wrapper(cfg, logger, skip, debug):
        ...
        if ('check_input' not in cfg or cfg['check_input']) and not os.path.exists(cfg['input_path']):
        ...
        if 'output_path' in cfg and os.path.exists(cfg['output_path']) and skip:
            logger.debug(f"Skip task {func.__name__}: output path already exist in {cfg['output_path']}")
            output = None
        else:
        ...
        if 'delete_input' in cfg and cfg['delete_input'] and os.path.exists(cfg['input_path']):
