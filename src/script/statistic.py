...
def main(cfg):
    all_valid_obj_num = len(os.listdir(cfg['data']['processed_folder']))
    ...
    for task_name, task_cfg in cfg['task'].items():
        if 'output_path' in task_cfg:
            ...
            curr_valid_obj_num = len(glob(os.path.join(cfg['data']['processed_folder'], '**', task_cfg['output_path'])))
    ...
    print('Total success rate: %.4f' % (curr_valid_obj_num / all_valid_obj_num))
    return 
